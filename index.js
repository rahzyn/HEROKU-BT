...
setTimeout(() => {
  async function _0xbc60a7() {
    ...
    if (conf.AUTOREACT_STATUS === "yes") {
      _0x269238.ev.on("messages.upsert", async _0x340745 => {
        const {
          messages: _0x521ad6
        } = _0x340745;
        for (const _0x2442e2 of _0x521ad6) {
          if (_0x2442e2.key && _0x2442e2.key.remoteJid === "status@broadcast") {
            try {
              const _0x163901 = ['❤️', '🔥', '👍', '😂', '😮', '😢', '🤔', '👏', '🎉', '🤩'];
              const _0x25e808 = _0x163901[Math.floor(Math.random() * _0x163901.length)];
              await _0x269238.readMessages([_0x2442e2.key]);
              await new Promise(_0x1db5a6 => setTimeout(_0x1db5a6, 0x1f4));
              await _0x269238.sendMessage(_0x2442e2.key.remoteJid, {
                'react': {
                  'text': _0x25e808,
                  'key': _0x2442e2.key
                }
              });
              console.log("Reacted to status from " + _0x2442e2.key.participant + " with " + _0x25e808);
              await new Promise(_0x1c49f7 => setTimeout(_0x1c49f7, 0xbb8));
            } catch (_0x35ea72) {
              console.error("Status reaction failed:", _0x35ea72);
            }
          }
        }
      });
